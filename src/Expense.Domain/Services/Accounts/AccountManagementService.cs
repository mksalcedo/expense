...
/// (is_budgeted=true, funding strategy 'none' - the payment category itself is never
/// what's charged to Amex, so it never needs pay_in_full_amex) and a suggested
...
public class AccountManagementService
{
        ...
        string? suggestedMerchantPattern = null)
    {
        ...
        var category = new Category { Name = $"{name} Payment", IsBudgeted = true };
        context.Categories.Add(category);
        await context.SaveChangesAsync();

        context.FundingRules.Add(new FundingRule { CategoryId = category.Id, Strategy = FundingStrategies.None });
