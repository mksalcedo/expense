...
public interface ICategoriesPageProvider
{
    ...
    Task CreateCategoryAsync(string name, string fundingStrategy, BudgetInput? budget = null, AccountPaymentInput? accountPayment = null, CancellationToken cancellationToken = default);

    Task UpdateCategoryAsync(int categoryId, string name, string fundingStrategy, bool reconcileByCalendarMonth = false, decimal? carryoverCapMultiplier = 1.0m, BudgetInput? budget = null, AccountPaymentInput? accountPayment = null, CancellationToken cancellationToken = default);
