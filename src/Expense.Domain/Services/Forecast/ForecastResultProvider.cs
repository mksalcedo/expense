    ...
    TransactionReconciliationService reconciliation, PaymentAmountAdjustmentService amountAdjustments) : IForecastResultProvider
{
    ...
    public async Task PayPartialAmountAsync(
        int accountId, DateOnly originalDate, DateOnly paidDate, decimal amount, Direction direction, string lineDescription, CancellationToken cancellationToken = default)
    {
        await using var context = await contextFactory.CreateDbContextAsync(cancellationToken);
        await partialPayments.CreateAsync(context, accountId, originalDate, paidDate, amount, direction, lineDescription, cancellationToken);
    }
