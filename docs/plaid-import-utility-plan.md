...
**Decision for this slice**: add one new, narrow method to `DedupService` - `ExistsForAccountDateAmountAsync(context, accountId, postedDate, amount)` - checking only `(AccountId, PostedDate, Amount)`, deliberately ignoring description/source. Used by both importers, layered *after* each one's own more-precise `ExternalId`/fingerprint check. Only applies when `PostedDate` is not null (pending transactions have nothing to collide with, since SimpleFin never reports pending rows).

**Correction (2026-07-27, real Amex rollout)**: this was originally one-directional - only the Plaid importer checked against SimpleFin, "SimpleFin's own import path is untouched." That gap let 5 real duplicate transactions into production: Plaid backfilled a stale SimpleFin connection on 2026-07-25, then the next morning's scheduled SimpleFin sync (2026-07-26) re-added the same transactions under its own ids, since it never checked whether Plaid already had them. Fixed by adding the same cross-source check to `SimpleFinImportService`'s own transaction loop - the check is now symmetric, not Plaid-only. The 5 real duplicate rows were identified and removed directly from the production database.
