...
**Fix (2026-08-12):** exclude any candidate link found inside `.closest('.a-carousel-card')`.
Verified two ways: against all five real saved order pages (still correct, no regression),
and against a synthetic HTML fixture built to exactly reproduce the reported live structure
(`#orderDetails` containing both the real item and an `#adfeedbackdetails` > `.a-carousel` >
`.a-carousel-card` block, matching the live diagnostic dump byte-for-byte in shape) - since
no real saved file has ever captured the carousel at all, this synthetic fixture is the only
way to regression-test this specific fix's correctness offline.

## Third bug, found live 2026-09-08: the open mini-cart flyout

Scraping a real order (`113-9957140-8010655`, a single API Pond test-strip kit, $14.98) with
the **shopping-cart flyout panel open** on the right side of the page copied 4 items - all of
them cart contents (photo paper, glitter ornaments) - and missed the one real order item
entirely. Cart items are `/dp/` links with a price right next to them and they aren't
carousel cards, so neither existing filter touched them.

A live diagnostic dump (the same ancestor-chain-dump bookmarklet approach as before) made
the discriminator obvious: **the real order item is the only `/dp//product/` link that is
inside `#orderDetails` AND not inside `.a-carousel-card`.** The mini-cart lives under
`#ewc-compact-body` / `.ewc-item` / `sc-item-*`, entirely outside `#orderDetails`. The
"Products related to your order" recommendation carousels are outside it too. The one
carousel that *is* inside `#orderDetails` ("Buy it again", `#anonCarousel1`) is still caught
by the `.a-carousel-card` exclusion.

**Fix:** scope the link search to `#orderDetails` **and** keep the `.a-carousel-card`
exclusion - the combination the earlier round stopped short of (it replaced container scoping
with the carousel exclusion rather than using both). If `#orderDetails` is absent the
bookmarklet returns nothing and the "found 0 items" alert fires, rather than falling back to
a whole-document scan. Verified via Node + jsdom against a synthetic fixture reproducing the
2026-09-08 diagnostic dump (cart flyout + in-`#orderDetails` carousel + rec carousel + footer
links, only the API Pond item expected out) plus the qty-3, two-item, single-item, and
no-`#orderDetails` cases.
...
- **Not yet re-confirmed with an actual live click** after the `#orderDetails`-scope +
  `.a-carousel-card` fix (2026-09-08). This is the third round of live fixes on this
  bookmarklet - treat "should be right this time" with appropriate skepticism until it's
  actually re-tried live (ideally on the same `113-9957140-8010655` order, cart flyout open).
...
(function () {
    ...
    function findItemBlocks() {
        // Three real bugs found live, in order:
        ...
        // 2. Every recommendation-carousel item, wherever it lands in the DOM, sits inside
        //    an <li class="a-carousel-card"> - Amazon's standard reusable carousel widget
        //    markup, used for "Products related to your order," "Buy it again," etc. The
        //    real order item never does.
        // 3. When the mini-cart flyout panel is open, its items (#ewc-compact-body /
        //    .ewc-item / sc-item-* ids) are also product links with a price right next to
        //    them, and they aren't carousel cards (found live 2026-09-08: 4 unrelated cart
        //    items scraped, the one real order item missed entirely).
        // The real order line item is always inside #orderDetails - a stable, server-
        //    rendered container present on every real order-details page. The mini-cart and
        //    the out-of-line recommendation carousels are not. Scope to #orderDetails, AND
        //    still exclude .a-carousel-card, because Amazon also injects a "Buy it again"
        //    carousel INSIDE #orderDetails (2026-08-12 diagnostic) whose cards container
        //    scoping alone wouldn't stop. A live DOM diagnostic dump on 2026-09-08 confirmed
        //    the real item is the only /dp//product/ link that is inside #orderDetails and
        //    not inside .a-carousel-card.
        const scope = document.querySelector('#orderDetails');
        if (!scope) return [];

        const links = Array.from(scope.querySelectorAll('a[href*="/product/"], a[href*="/dp/"]'))
    ...
    if (items.length === 0) {
        alert('Amazon order scraper: found 0 items on this page. The page layout may not ' +
            'match what this bookmarklet expects (it looks for product links inside the ' +
            'page\'s #orderDetails container) - report this back so the selectors can be ' +
            'adjusted.');
        return;
    }
...
```
javascript:%28function%20%28%29%20%7B%0A%20%20%20%20%27use%20strict%27%3B%0A%0A%20%20%20%20function%20extractOrderId%28%29%20%7B%0A%20%20%20%20%20%20%20%20const%20params%20%3D%20new%20URLSearchParams%28window.location.search%29%3B%0A%20%20%20%20%20%20%20%20return%20params.get%28%27orderID%27%29%20%7C%7C%20params.get%28%27orderId%27%29%20%7C%7C%20null%3B%0A%20%20%20%20%7D%0A%0A%20%20%20%20function%20parsePrice%28text%29%20%7B%0A%20%20%20%20%20%20%20%20const%20match%20%3D%20text.match%28/%5C%24%28%5B%5Cd%2C%5D%2B%5C.%5Cd%7B2%7D%29/%29%3B%0A%20%20%20%20%20%20%20%20return%20match%20%3F%20parseFloat%28match%5B1%5D.replace%28/%2C/g%2C%20%27%27%29%29%20%3A%20null%3B%0A%20%20%20%20%7D%0A%0A%20%20%20%20//%20Amazon%27s%20order-details%20page%20layout%20varies%20%28regular%20physical%20orders%2C%20digital%20orders%2C%0A%20%20%20%20//%20Subscribe%20%26%20Save%2C%20etc.%29%20and%20changes%20over%20time%20without%20notice%20-%20this%20looks%20for%20the%0A%20%20%20%20//%20most%20common%20pattern%3A%20each%20real%20item%20is%20a%20product%20link%20%28href%20containing%20%22/product/%22%0A%20%20%20%20//%20or%20%22/dp/%22%29%20with%20a%20price%20and%20a%20quantity%20indicator%20somewhere%20in%20the%20same%20surrounding%0A%20%20%20%20//%20block.%20Verified%20against%20five%20real%20saved/diagnosed%20order%20pages%20%282026-08-11/12%29%3A%0A%20%20%20%20//%20quantity%20is%20NOT%20shown%20as%20%22Qty%3A%20N%22%20text%20anywhere%20-%20it%27s%20a%20bare%20number%20inside%20an%0A%20%20%20%20//%20element%20whose%20class%20contains%20%22qty%22%20%28e.g.%0A%20%20%20%20//%20%3Cdiv%20class%3D%22od-item-view-qty%22%3E%3Cspan%3E3%3C/span%3E%3C/div%3E%29%2C%20which%20is%20why%20this%20reads%0A%20%20%20%20//%20quantity%20from%20that%20element%20directly%20rather%20than%20by%20matching%20visible%20text.%20A%0A%20%20%20%20//%20single-item%20order%20has%20no%20such%20element%20at%20all%2C%20so%20quantity%20correctly%20falls%20back%20to%201%0A%20%20%20%20//%20when%20none%20is%20found.%0A%20%20%20%20function%20findItemBlocks%28%29%20%7B%0A%20%20%20%20%20%20%20%20//%20Three%20real%20bugs%20found%20live%2C%20in%20order%3A%0A%20%20%20%20%20%20%20%20//%201.%20%22Products%20related%20to%20your%20order%22%20%28a%20recommendation%20carousel%29%20loads%20via%20JS%0A%20%20%20%20%20%20%20%20//%20%20%20%20after%20the%20initial%20page%20render%2C%20so%20it%27s%20invisible%20to%20every%20saved-HTML%20test%20-%0A%20%20%20%20%20%20%20%20//%20%20%20%20only%20a%20real%20live%20click%20ever%20showed%20it%20%28found%20live%202026-08-12%3A%2028%20items%20copied%2C%0A%20%20%20%20%20%20%20%20//%20%20%20%2027%20from%20the%20carousel%29.%0A%20%20%20%20%20%20%20%20//%202.%20Every%20recommendation-carousel%20item%2C%20wherever%20it%20lands%20in%20the%20DOM%2C%20sits%20inside%0A%20%20%20%20%20%20%20%20//%20%20%20%20an%20%3Cli%20class%3D%22a-carousel-card%22%3E%20-%20Amazon%27s%20standard%20reusable%20carousel%20widget%0A%20%20%20%20%20%20%20%20//%20%20%20%20markup%2C%20used%20for%20%22Products%20related%20to%20your%20order%2C%22%20%22Buy%20it%20again%2C%22%20etc.%20The%0A%20%20%20%20%20%20%20%20//%20%20%20%20real%20order%20item%20never%20does.%0A%20%20%20%20%20%20%20%20//%203.%20When%20the%20mini-cart%20flyout%20panel%20is%20open%2C%20its%20items%20%28%23ewc-compact-body%20/%0A%20%20%20%20%20%20%20%20//%20%20%20%20.ewc-item%20/%20sc-item-%2A%20ids%29%20are%20also%20product%20links%20with%20a%20price%20right%20next%20to%0A%20%20%20%20%20%20%20%20//%20%20%20%20them%2C%20and%20they%20aren%27t%20carousel%20cards%20%28found%20live%202026-09-08%3A%204%20unrelated%20cart%0A%20%20%20%20%20%20%20%20//%20%20%20%20items%20scraped%2C%20the%20one%20real%20order%20item%20missed%20entirely%29.%0A%20%20%20%20%20%20%20%20//%20The%20real%20order%20line%20item%20is%20always%20inside%20%23orderDetails%20-%20a%20stable%2C%20server-%0A%20%20%20%20%20%20%20%20//%20%20%20%20rendered%20container%20present%20on%20every%20real%20order-details%20page.%20The%20mini-cart%20and%0A%20%20%20%20%20%20%20%20//%20%20%20%20the%20out-of-line%20recommendation%20carousels%20are%20not.%20Scope%20to%20%23orderDetails%2C%20AND%0A%20%20%20%20%20%20%20%20//%20%20%20%20still%20exclude%20.a-carousel-card%2C%20because%20Amazon%20also%20injects%20a%20%22Buy%20it%20again%22%0A%20%20%20%20%20%20%20%20//%20%20%20%20carousel%20INSIDE%20%23orderDetails%20%282026-08-12%20diagnostic%29%20whose%20cards%20container%0A%20%20%20%20%20%20%20%20//%20%20%20%20scoping%20alone%20wouldn%27t%20stop.%20A%20live%20DOM%20diagnostic%20dump%20on%202026-09-08%20confirmed%0A%20%20%20%20%20%20%20%20//%20%20%20%20the%20real%20item%20is%20the%20only%20/dp//product/%20link%20that%20is%20inside%20%23orderDetails%20and%0A%20%20%20%20%20%20%20%20//%20%20%20%20not%20inside%20.a-carousel-card.%0A%20%20%20%20%20%20%20%20const%20scope%20%3D%20document.querySelector%28%27%23orderDetails%27%29%3B%0A%20%20%20%20%20%20%20%20if%20%28%21scope%29%20return%20%5B%5D%3B%0A%0A%20%20%20%20%20%20%20%20const%20links%20%3D%20Array.from%28scope.querySelectorAll%28%27a%5Bhref%2A%3D%22/product/%22%5D%2C%20a%5Bhref%2A%3D%22/dp/%22%5D%27%29%29%0A%20%20%20%20%20%20%20%20%20%20%20%20.filter%28a%20%3D%3E%20a.textContent.trim%28%29.length%20%3E%208%29%0A%20%20%20%20%20%20%20%20%20%20%20%20.filter%28a%20%3D%3E%20%21a.closest%28%27.a-carousel-card%27%29%29%3B%0A%0A%20%20%20%20%20%20%20%20const%20seen%20%3D%20new%20Set%28%29%3B%0A%20%20%20%20%20%20%20%20const%20items%20%3D%20%5B%5D%3B%0A%0A%20%20%20%20%20%20%20%20for%20%28const%20link%20of%20links%29%20%7B%0A%20%20%20%20%20%20%20%20%20%20%20%20const%20title%20%3D%20link.textContent.trim%28%29%3B%0A%20%20%20%20%20%20%20%20%20%20%20%20if%20%28seen.has%28title%29%29%20continue%3B%0A%0A%20%20%20%20%20%20%20%20%20%20%20%20//%20Walk%20up%20a%20few%20ancestor%20levels%20looking%20for%20a%20price%20and%20quantity%20nearby%20-%0A%20%20%20%20%20%20%20%20%20%20%20%20//%20Amazon%20nests%20the%20actual%20price/qty%20in%20a%20sibling%20block%2C%20not%20right%20next%20to%20the%0A%20%20%20%20%20%20%20%20%20%20%20%20//%20link%20itself.%0A%20%20%20%20%20%20%20%20%20%20%20%20let%20block%20%3D%20link%3B%0A%20%20%20%20%20%20%20%20%20%20%20%20let%20priceText%20%3D%20null%3B%0A%20%20%20%20%20%20%20%20%20%20%20%20let%20quantity%20%3D%20null%3B%0A%20%20%20%20%20%20%20%20%20%20%20%20for%20%28let%20i%20%3D%200%3B%20i%20%3C%206%20%26%26%20block%3B%20i%2B%2B%29%20%7B%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20block%20%3D%20block.parentElement%3B%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20if%20%28%21block%29%20break%3B%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20const%20text%20%3D%20block.textContent%3B%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20if%20%28%21priceText%20%26%26%20/%5C%24%5B%5Cd%2C%5D%2B%5C.%5Cd%7B2%7D/.test%28text%29%29%20priceText%20%3D%20text%3B%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20if%20%28quantity%20%3D%3D%3D%20null%29%20%7B%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20const%20qtyEl%20%3D%20block.querySelector%28%27%5Bclass%2A%3D%22qty%22%20i%5D%27%29%3B%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20if%20%28qtyEl%29%20%7B%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20const%20qtyMatch%20%3D%20qtyEl.textContent.match%28/%5Cd%2B/%29%3B%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20if%20%28qtyMatch%29%20quantity%20%3D%20parseInt%28qtyMatch%5B0%5D%2C%2010%29%3B%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%7D%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%7D%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20if%20%28priceText%20%26%26%20quantity%20%21%3D%3D%20null%29%20break%3B%0A%20%20%20%20%20%20%20%20%20%20%20%20%7D%0A%0A%20%20%20%20%20%20%20%20%20%20%20%20const%20price%20%3D%20priceText%20%3F%20parsePrice%28priceText%29%20%3A%20null%3B%0A%20%20%20%20%20%20%20%20%20%20%20%20//%20No%20price%20found%20nearby%20-%20likely%20a%20footer/promotional%20link%2C%20not%20a%20real%20line%0A%20%20%20%20%20%20%20%20%20%20%20%20//%20item%20in%20this%20order.%20Skip%20it%20rather%20than%20guess.%0A%20%20%20%20%20%20%20%20%20%20%20%20if%20%28price%20%3D%3D%3D%20null%29%20continue%3B%0A%0A%20%20%20%20%20%20%20%20%20%20%20%20seen.add%28title%29%3B%0A%20%20%20%20%20%20%20%20%20%20%20%20items.push%28%7B%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20title%3A%20title%2C%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20price%3A%20price%2C%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20quantity%3A%20quantity%20%21%3D%3D%20null%20%3F%20quantity%20%3A%201%0A%20%20%20%20%20%20%20%20%20%20%20%20%7D%29%3B%0A%20%20%20%20%20%20%20%20%7D%0A%0A%20%20%20%20%20%20%20%20return%20items%3B%0A%20%20%20%20%7D%0A%0A%20%20%20%20const%20orderId%20%3D%20extractOrderId%28%29%3B%0A%20%20%20%20const%20items%20%3D%20findItemBlocks%28%29%3B%0A%0A%20%20%20%20if%20%28items.length%20%3D%3D%3D%200%29%20%7B%0A%20%20%20%20%20%20%20%20alert%28%27Amazon%20order%20scraper%3A%20found%200%20items%20on%20this%20page.%20The%20page%20layout%20may%20not%20%27%20%2B%0A%20%20%20%20%20%20%20%20%20%20%20%20%27match%20what%20this%20bookmarklet%20expects%20%28it%20looks%20for%20product%20links%20inside%20the%20%27%20%2B%0A%20%20%20%20%20%20%20%20%20%20%20%20%27page%5C%27s%20%23orderDetails%20container%29%20-%20report%20this%20back%20so%20the%20selectors%20can%20be%20%27%20%2B%0A%20%20%20%20%20%20%20%20%20%20%20%20%27adjusted.%27%29%3B%0A%20%20%20%20%20%20%20%20return%3B%0A%20%20%20%20%7D%0A%0A%20%20%20%20const%20payload%20%3D%20JSON.stringify%28%7B%20orderId%3A%20orderId%2C%20items%3A%20items%20%7D%29%3B%0A%20%20%20%20navigator.clipboard.writeText%28payload%29.then%28function%20%28%29%20%7B%0A%20%20%20%20%20%20%20%20const%20summary%20%3D%20items.map%28function%20%28i%29%20%7B%0A%20%20%20%20%20%20%20%20%20%20%20%20return%20%27-%20%27%20%2B%20i.title%20%2B%20%27%20%28%24%27%20%2B%20i.price.toFixed%282%29%20%2B%20%27%20x%27%20%2B%20i.quantity%20%2B%20%27%29%27%3B%0A%20%20%20%20%20%20%20%20%7D%29.join%28%27%5Cn%27%29%3B%0A%20%20%20%20%20%20%20%20alert%28%27Copied%20%27%20%2B%20items.length%20%2B%20%27%20item%28s%29%20to%20clipboard%3A%5Cn%5Cn%27%20%2B%20summary%20%2B%0A%20%20%20%20%20%20%20%20%20%20%20%20%27%5Cn%5CnNow%20switch%20to%20the%20Review%20Queue%20tab%20and%20paste%20%28Ctrl%2BV%29%20into%20the%20item%5C%27s%20paste%20target.%27%29%3B%0A%20%20%20%20%7D%29.catch%28function%20%28err%29%20%7B%0A%20%20%20%20%20%20%20%20alert%28%27Found%20%27%20%2B%20items.length%20%2B%20%27%20item%28s%29%20but%20could%20not%20copy%20to%20clipboard%3A%20%27%20%2B%20err.message%29%3B%0A%20%20%20%20%7D%29%3B%0A%7D%29%28%29%3B%0A
```
...
Built 2026-08-11. Fixed three times after real live-click bugs: the recommendation carousel
(2026-08-12, `.a-carousel-card` exclusion, from a live DOM diagnostic dump), and the open
mini-cart flyout (2026-09-08, scope to `#orderDetails` **and** keep `.a-carousel-card`, again
from a live diagnostic dump). Each fix verified via Node + jsdom running the actual script
end-to-end against synthetic fixtures reproducing the live diagnostic dumps, plus the qty-3 /
two-item / single-item / no-container cases. **Not yet re-confirmed with a live click since
the 2026-09-08 fix** - see "What's still unverified".
