...
**Wiring**: call `TransactionReconciliationService.ReconcileAsync` from `SyncScheduler.RunScheduledSyncAsync`, right alongside the existing `ForecastSnapshot` capture (same scope, same twice-daily cadence) - newly-synced and newly-categorized transactions get classified automatically without a third `BackgroundService`.
