...
public class PartialPaymentServiceTests : DatabaseTestBase
{
    ...
    public async Task CreateAsync_ForAnExpense_RecordsTheRealCashMovementAsAnExpenseOneTimeEvent()
    {
        ...
        var partialPayment = await _sut.CreateAsync(Context, account.Id, new DateOnly(2026, 7, 20), new DateOnly(2026, 7, 14), 1000m, Direction.Expense);
    ...
    public async Task CreateAsync_ForIncome_RecordsTheRealCashMovementAsAnIncomeOneTimeEvent()
    {
        ...
        var partialPayment = await _sut.CreateAsync(Context, account.Id, new DateOnly(2026, 8, 5), new DateOnly(2026, 7, 22), 429m, Direction.Income);
